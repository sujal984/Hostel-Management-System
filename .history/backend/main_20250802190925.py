...
def forget_password(data: AdminData):
    ...
    try:
        ...
        user_query = admin.select().where(admin.c.User_Name == data.username)
        ...
        update_query = admin.update().where(admin.c.User_Name == data.username).values(Password=data.password)
