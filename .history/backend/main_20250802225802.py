...
if DATABASE_URL:
    ...
    # Add .internal if missing (for Render internal networking)
    if "render.com" in DATABASE_URL and not DATABASE_URL.endswith(".internal"):
        parts = DATABASE_URL.split("@")
        if len(parts) == 2:
            host_part = parts[1].split("/")[0]
            if not host_part.endswith(".internal"):
                DATABASE_URL = DATABASE_URL.replace(host_part, f"{host_part}.internal")
...
try:
    ...
    # Move table creation to after connection is verified
